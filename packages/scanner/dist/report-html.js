...
function badge(b) {
    if (b === 'high')
        return '<span aria-label="Widely Baseline" title="Widely Baseline" class="b hi">Widely</span>';
    if (b === 'low')
        return '<span aria-label="Newly Baseline" title="Newly Baseline" class="b lo">Newly</span>';
    if (b === false)
        return '<span aria-label="Not in Baseline" title="Not in Baseline" class="b no">Not in</span>';
...
function toCSV(items) {
    ...
    const rows = items.map((i) => [i.file, i.loc, i.bcdKey, i.featureName ?? '', i.baseline ?? '', i.mdn_url ?? '', (i.advice ?? '').replace(/\s+/g, ' ')]
        .map((v) => `"${String(v).replace(/"/g, '""')}"`)
        .join(','));
    return [header.join(','), ...rows].join('\n');
}
async function main() {
    const src = node_path_1.default.resolve(process.cwd(), 'report.json');
    const dstHtml = node_path_1.default.resolve(process.cwd(), 'report.html');
    const dstCsv = node_path_1.default.resolve(process.cwd(), 'report.csv');
    const report = JSON.parse(await promises_1.default.readFile(src, 'utf8'));
...
<script>
  ...
  applyFilters();
    ...
    await promises_1.default.writeFile(dstHtml, html, 'utf8');
    await promises_1.default.writeFile(dstCsv, toCSV(report.items), 'utf8');
    console.log(`Wrote ${node_path_1.default.basename(dstHtml)} and ${node_path_1.default.basename(dstCsv)}`);
}
main().catch((e) => {
    console.error(e);
    process.exit(1);
