...
async function main() {
    ...
    {
        const res = (0, node_child_process_1.spawnSync)(process.execPath, // 'node'
        [node_path_1.default.join(__dirname, 'index.js'), targetRoot], { stdio: 'inherit', cwd: tmp });
        if (res.status !== 0)
            die('Scan failed.');
    }
    // 2) build HTML/CSV in same temp dir
    {
        const res = (0, node_child_process_1.spawnSync)(process.execPath, [node_path_1.default.join(__dirname, 'report-html.js')], { stdio: 'inherit', cwd: tmp });
        if (res.status !== 0)
